    ...
    renderGeolocationWeather(data) {
      this._el.innerHTML = `
          <div class="custom-container">
            <h3 class="geo-title">You are in <span class="highlight">${data.name}</span>. Outside:</h3>
            <table class="table borderless">
                <thead>
                </thead>
                <tbody>
                    <tr><td>${data.weather[0].description}</td></tr>
                    <tr><td>${data.main.temp.toFixed()}°C</td></tr>
                    <tr><td>${data.wind.speed} m/s</td></tr>
                    <tr><td>${(data.main.pressure / 1.33).toFixed()} mmHg</td></tr>
                </tbody>
            </table>
      `;
    }
